...
// Función para encontrar números primos en un rango dado
function encontrarPrimosEnRango($inicio, $fin) {
    // Inicializar arreglos para almacenar los números primos, primos pares e impares
    $todosPrimos = [];
    $primosPares = [];
    $primosImpares = [];

    // Validar que los valores de inicio y fin sean enteros mayores que 1
    ...
    // Iterar a través del rango y verificar cada número
    for ($numero = $inicio; $numero <= $fin; $numero++) {
        list($esPrimo, $paridad) = verificarNumero($numero);
        
        if ($esPrimo) {
            // Añadir el número a la lista de todos los números primos
            $todosPrimos[] = $numero;
